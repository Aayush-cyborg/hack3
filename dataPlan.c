...
int main(int argc, char **argv) {
     ...
     printf("Enter the Number of GB in plan : "); // input from user the amount of data
     scanf("%f",&num);

     printf("Enter the number of days remaining from 1 to 30 : "); // input from user the number of remaining days
     scanf("%d",&days);

     printf("Enter the Amount of data used till now : "); // input from user the amount of data used
     scanf("%f",&used);

     float given =num/30;   //calculating the desired average of data that should be used
     float avg=used/(30-days);    //average of data that consumer is using
